...
exports.getLiquidacionById = exports.getLiquidacionesByPeriodo = void 0;
exports.generarLiquidaciones = generarLiquidaciones;
exports.aprobarLiquidacion = aprobarLiquidacion;
...
Object.defineProperty(exports, "getLiquidacionesByPeriodo", { enumerable: true, get: function () { return liquidaciones_repo_js_1.getLiquidacionesByPeriodo; } });
Object.defineProperty(exports, "getLiquidacionById", { enumerable: true, get: function () { return liquidaciones_repo_js_1.getLiquidacionById; } });
// ─── Helpers ──────────────────────────────────────────────────────────────────
...
async function generarLiquidaciones(fechaDesde, fechaHasta) {
    const resultado = await (0, honorarios_service_js_1.calcularHonorariosRango)(fechaDesde, fechaHasta);
    for (const row of resultado.rows) {
        await (0, liquidaciones_repo_js_1.upsertLiquidacion)({
...
// ─── Aprobar ──────────────────────────────────────────────────────────────────
async function aprobarLiquidacion(id, usuarioId) {
    await (0, liquidaciones_repo_js_1.actualizarEstado)(id, 'APROBADO', usuarioId);
    return (0, liquidaciones_repo_js_1.getLiquidacionById)(id);
}
async function aprobarLote(ids, usuarioId) {
    await (0, liquidaciones_repo_js_1.actualizarEstadoLote)(ids, 'APROBADO', usuarioId);
}
// ─── Pagar ────────────────────────────────────────────────────────────────────
async function pagarLiquidacion(id, usuarioId, notas) {
    await (0, liquidaciones_repo_js_1.actualizarEstado)(id, 'PAGADO', usuarioId, notas);
    return (0, liquidaciones_repo_js_1.getLiquidacionById)(id);
}
...
async function generarPDFLiquidacion(id) {
    const liq = await (0, liquidaciones_repo_js_1.getLiquidacionById)(id);
    if (!liq)
    ...
    return new Promise((resolve, reject) => {
        ...
        const infoY = 158;
        const col2 = X + W / 2;
        const infoRow = (label, value, y, col = X) => {
        ...
        if (liq.especialidad)
            infoRow('Especialidad', liq.especialidad, infoY + 36);
        ...
        for (const { key, label } of CATS) {
            const celda = snapshot[key];
            if (!celda || celda.monto === 0)
                continue;
            if (rowIdx % 2 === 0) {
        ...
        doc.text(fmtCOP(liq.monto_total), X + colWidths[0] + colWidths[1], rowY + 6, { width: colWidths[2] - 8, align: 'right' });
        rowY += 24;
