...
exports.autoSeedReglasHonorarios = autoSeedReglasHonorarios;
...
async function autoSeedReglasHonorarios() {
    const existingReglas = await (0, reglas_honorarios_repo_js_1.countReglas)();
    const existingEspeciales = await (0, reglas_honorarios_repo_js_1.countReglasEspeciales)();
    if (existingReglas >= REGLAS_SEED.length && existingEspeciales >= ESPECIALES_SEED.length) {
        logger_js_1.logger.info('Reglas honorarios seed: ya está completo, omitiendo');
        return;
    }
    if (existingReglas < REGLAS_SEED.length) {
        for (const r of REGLAS_SEED) {
            await (0, reglas_honorarios_repo_js_1.upsertRegla)(r.prof, r.cat, r.tipo, r.entidad, r.particular, r.notas ?? null);
        }
        logger_js_1.logger.info(`Reglas honorarios seed: ${REGLAS_SEED.length} reglas insertadas/actualizadas`);
    }
    if (existingEspeciales < ESPECIALES_SEED.length) {
        for (const e of ESPECIALES_SEED) {
            await (0, reglas_honorarios_repo_js_1.insertReglaEspecial)(e.tipo_regla, e.prof, e.condicion, e.valor, e.descripcion);
        }
        logger_js_1.logger.info(`Reglas especiales seed: ${ESPECIALES_SEED.length} reglas especiales insertadas`);
    }
}
//# sourceMappingURL=reglas-honorarios-seed.service.js.map