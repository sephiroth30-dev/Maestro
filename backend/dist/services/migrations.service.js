...
async function runEntityMigration() {
    try {
        ...
        for (const [nombre, nombresRaw] of UPDATED_ENTITIES) {
            await prisma_js_1.pool.execute('UPDATE entidades SET nombres_raw = ? WHERE nombre = ?', [nombresRaw, nombre]);
        }
        // 2. Remove duplicate 'PARTICULAR' entry if it exists alongside 'PARTICULARES'
        const [particularRows] = await prisma_js_1.pool.query("SELECT id FROM entidades WHERE nombre = 'PARTICULAR' LIMIT 1");
        const [particularesRows] = await prisma_js_1.pool.query("SELECT id FROM entidades WHERE nombre = 'PARTICULARES' LIMIT 1");
        if (particularRows.length > 0 && particularesRows.length > 0) {
            await prisma_js_1.pool.execute("DELETE FROM entidades WHERE nombre = 'PARTICULAR'");
        }
        // 3. Insert new entities (skip if nombre already exists)
        const [existing] = await prisma_js_1.pool.query('SELECT nombre FROM entidades');
        ...
        for (const [nombre, nombresRaw, tipo, esGrupoCaja] of NEW_ENTITIES) {
            if (!existingNames.has(nombre.toUpperCase())) {
                await prisma_js_1.pool.execute('INSERT INTO entidades (id, nombre, nombres_raw, tipo, es_grupo_caja, activa, created_at) VALUES (?, ?, ?, ?, ?, 1, NOW())', [(0, node_crypto_1.randomUUID)(), nombre, nombresRaw, tipo, esGrupoCaja]);
                inserted++;
            }
        }
        logger_js_1.logger.info('Entity migration complete', { inserted, updated: UPDATED_ENTITIES.length });
    }
    catch (err) {
        logger_js_1.logger.error('Entity migration failed', { error: err.message });
    }
}
